...
class PlanningEngine:
    ...
    User Input Parameters (from procedure document):
    - planning_month: Which month the S&OP cycle is based on
    - months_actuals: How many months of actual sales data in forecast
    - months_forecast: Planning horizon (typically 12 months)
    ...
    def __init__(self, file_path: str, planning_month: str = None, months_actuals: int = 0, months_forecast: int = 12):
        """
        Initialize Planning Engine.
        
        Args:
            file_path: Path to Excel file
            planning_month: Which month the planning is based on (e.g., "2025-04")
            months_actuals: How many months of actuals are in the forecast sheet
            months_forecast: Planning horizon in months (default 12)
        """
    ...
    def run(self) -> 'PlanningEngine':
        ...
        print(f"  Planning Month: {self.planning_month or 'Auto-detect'}")
        print(f"  Months of Actuals: {self.months_actuals}")
        print(f"  Months of Forecast: {self.months_forecast}")
        ...
        # Step 2: Calculate demand forecast with user parameters
        print("\n[STEP 2] Running calculations...")
        forecast_engine = ForecastEngine(
            self.data, 
            months_actuals=self.months_actuals,
